...
class Effects implements EffectsInterface
{
    ...
    public function brighten($degree) {
        if (false === imagefilter($this->resource, IMG_FILTER_BRIGHTNESS, $degree)) {
            throw new RuntimeException('Failed to brighten the image');
        }

        return $this;
    }

    public function contrast($degree) {
        if (false === imagefilter($this->resource, IMG_FILTER_CONTRAST, $degree)) {
    ...
    public function edge_detect() {
        if (false === imagefilter($this->resource, IMG_FILTER_EDGEDETECT)) {
            throw new RuntimeException('Failed to apply edge detection on the image');
        }
